...
def check_smartphones():

    smart_api = 'http://127.0.0.1:8000/phone/check_phone'
    st.title('Smart Phone ')
    st.write('Загрузите изображение вашего смартфона (телефон)')
    upload_file = st.file_uploader('Выберите изображение')
    if upload_file is not None:
        st.image(upload_file, caption='Загруженное изображение')
        if st.button('Определить марку смартфона'):
            try:
                file = {
