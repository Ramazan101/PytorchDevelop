...
from ALL_ML.database.module import ChechImage2
...
check_smartphone = APIRouter(prefix='/phone', tags=['Smart Phone'])
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')


BASE_DIR = Path(__file__).resolve().parent
MODEL_DIR = BASE_DIR.parent / 'all_models'
model_path = MODEL_DIR / 'model_phone.pth'
...
model.to(device)
model.eval()
classes = ['huawei', 'iphone', 'poco', 'redmi', 'samsung']

@check_smartphone.post('/check_phone')
